...
describe('app', () => {
  ...
  it('should return 404 for routes with not found id', () => {
    return superagent
      .get('http://localhost:3003/api/v1/notes/123')
      .catch(err => {
  ...
  it('should return 200 and contain response body for request made with valid id', () => {
    ...
    return superagent
      ...
      .then(data => {
        return superagent
          .get(`http://localhost:3003/api/v1/notes/${data.body.id}`)
          .then(response => {
  ...
  it('should  respond with the body content', () => {
    let object = { 'name': 'one name', 'doggos': 'such doggos'};
    return superagent
      .post('http://localhost:3003/api/v1/notes')
      .send(object)
      .then(data => {
        expect(data.body.doggos).toBe('such doggos');
      });
  });
});